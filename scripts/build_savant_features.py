...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build/import local Baseball Savant-style feature rows.")
    parser.add_argument("--dataset", choices=sorted(PLAYER_METRIC_DATASETS), default="statcast_batter_daily")
    ...
    normalized = container.baseball_savant_feature_service.normalize_rows(rows, dataset=args.dataset, date_label=args.date)
    if args.dry_run:
        print(json.dumps({"status": "ok", "dataset": args.dataset, "dry_run": True, "row_count": len(normalized)}, indent=2))
        return 0

    result = container.baseball_savant_feature_service.upsert_rows(
        args.dataset,
        rows,
