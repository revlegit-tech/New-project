...
try:
    from mlb_app.domain.build_game_odds_template import TEAM_NAME_TO_ABBR as MLB_TEAM_NAME_TO_ABBR
except Exception:
...
def saved_games_payload(date_label: str) -> dict[str, Any]:
    ...
    if not rows:
        try:
            from mlb_app.integrations.statsapi.warehouse_sync import sync_mlb_schedule
