...
from mlb_app.api.dependencies import get_alert_service, get_app_status_service, get_metrics_registry, get_model_registry_service, query_params, request_id
...
@router.get("/metrics", name="native_observability_metrics")
async def metrics(registry: Annotated[MetricsRegistry, Depends(get_metrics_registry)]) -> dict[str, Any]:
    return await asyncio.to_thread(registry.snapshot)
