...
def find_odds_movement_context(
    ...
    try:
        from mlb_app.domain.odds_movement import find_movement
...
def player_id_from_index(player: str, season: int) -> str:
    if not clean(player):
        return ""
    try:
        from mlb_app.domain.savant_features import load_player_index
...
def raw_statcast_rows(season: int) -> list[dict[str, str]]:
    try:
        from mlb_app.domain.savant_features import latest_raw_file, read_csv_rows
...
def find_savant_context(season: int, player: str, pitcher: str, market: str) -> dict[str, Any]:
    ...
    try:
        from mlb_app.domain.savant_features import lookup_batter, lookup_pitcher
