...
from scripts.backtest_baseline_model import main as backtest_main
...
def test_backtest_status_missing_when_artifact_absent(tmp_path: Path) -> None:
    payload = ModelBacktestService(make_settings(tmp_path)).status(date_label="2026-06-24", season=2026, market="batter_hits")

    assert payload["schemaVersion"] == "model-backtest-status.v1"
    assert payload["artifactExists"] is False
    assert payload["backtestStatus"] == "missing"
    assert payload["modelTrainingTriggered"] is False
    assert payload["externalApiCallsMade"] is False


def test_backtest_dry_run_scores_model_artifact_feature_rows_and_labels(tmp_path: Path) -> None:
    settings = make_settings(tmp_path)
    artifact_dir = write_model_fixture(settings)

    payload = ModelBacktestService(settings).backtest(date_label="2026-06-22", season=2026, market="batter_total_bases", backtest=False)

    assert payload["artifactWritten"] is False
    assert payload["metrics"]["evaluatedRows"] == 220
    assert payload["metrics"]["hitRate"] is not None
    assert payload["backtestStatus"] == "ready"
    assert payload["warnings"] == []
    assert not (artifact_dir / "backtest_metrics.json").exists()
    assert not (artifact_dir / "backtest_by_bucket.csv").exists()
    assert not (artifact_dir / "backtest_manifest.json").exists()
    assert payload["modelTrainingTriggered"] is False
    assert payload["externalApiCallsMade"] is False


def test_backtest_writes_artifacts_when_requested(tmp_path: Path) -> None:
    settings = make_settings(tmp_path)
    artifact_dir = write_model_fixture(settings)

    payload = ModelBacktestService(settings).backtest(date_label="2026-06-22", season=2026, market="batter_total_bases", backtest=True)

    assert payload["artifactWritten"] is True
    assert payload["backtestStatus"] == "ready"
    assert (artifact_dir / "backtest_metrics.json").is_file()
    assert (artifact_dir / "backtest_by_bucket.csv").is_file()
    assert (artifact_dir / "backtest_manifest.json").is_file()


def test_backtest_missing_model_returns_existing_warning(tmp_path: Path) -> None:
    settings = make_settings(tmp_path)
...
def test_backtest_missing_feature_columns_returns_clear_warning(tmp_path: Path) -> None:
    settings = make_settings(tmp_path)
    artifact_dir = write_model_fixture(settings)
...
def test_backtest_preserves_precomputed_probability_rows_fallback(tmp_path: Path) -> None:
    settings = make_settings(tmp_path)
    ...
    payload = ModelBacktestService(settings).backtest(date_label="2026-06-22", season=2026, market="batter_hits", backtest=False)

    assert payload["metrics"]["evaluatedRows"] == 20
    assert not any("Baseline model scoring failed" in warning for warning in payload["warnings"])


def test_backtest_dry_run_script_does_not_write_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    assert backtest_main(["--season", "2026", "--market", "batter_hits", "--dry-run"]) == 0
